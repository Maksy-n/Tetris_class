...
void Draw::OnScreen(Glass &toPrintGlass) // основной стакан, старт с линии 2
{
...
void Draw::OnScreen(Tetramina &nextFig, int randNext) // следующая фигура, сбоку
{
	for (int i = 0; i < 4; i++) 
	{
...
void Draw::OnScreen(int score) // счёт 
{
	Font font;
	...
	text.setPosition(240, 20);//позиция текста
	window.draw(text);	
}

void Draw::ShowOn() // вывод на экран всех методов
{
	window.display();
}
